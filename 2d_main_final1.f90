program main
    ...
    integer, parameter :: nx = 2**9 , ny = nx
    real(kind=8), parameter :: x_min = -3.0d0, x_max = -x_min
    real(kind=8), parameter :: y_min = -3.0d0, y_max = -y_min
    integer, parameter :: it_min = 1, it_max = 600000
  
    real(kind=8) :: dx, dy, dt, time, eta, total_mass, c, deltaeta
    integer :: i, j, it, test
 ...
 !-------------------------------------------------------------------
  ...
  !-------------------------------------------------------------------
    real(kind=8) :: psi(nx,ny), dens(nx,ny), ux(nx,ny), uy(nx,ny), ut(nx,ny)
    ...
    ! real(kind=8) :: q01_lxL(nx,ny), q01_rxL(nx,ny), q01_lyL(nx,ny), q01_ryL(nx,ny)
...
!---------------------------------------
  ...
  !test = 1 -> KH-Instability (ny=2*nx=2**9, Lx=0.5d0, Ly=1.5d0)
  !test = 2,3 -> Oblique-shockwave (nx=ny=2**9, L=200)
  !test = 4 -> gausiana

  eta=0.01d0
  deltaeta=0.0d0 !tolerancia
  c=1.0d0
...
!------------------------------------------------------------
  ...
  do j = 3, ny-3
    do i = 3, nx-3
      ...
      ut_rx(i,j) = sqrt(1.0d0 + ux_rx(i,j)**2 + uy_rx(i,j)**2)
      ut_lx(i,j) = sqrt(1.0d0 + ux_lx(i,j)**2 + uy_lx(i,j)**2)
      ut_ly(i,j) = sqrt(1.0d0 + ux_ly(i,j)**2 + uy_ly(i,j)**2)
      ut_ry(i,j) = sqrt(1.0d0 + ux_ry(i,j)**2 + uy_ry(i,j)**2)
    end do
  end do
...
!------------------------------------------------------------
    ...
    call flujos_xR(nx, ny, psi_lx, psi_rx, ux_lx, ux_rx, uy_lx, uy_rx, ut_lx, ut_rx, A_lx, A_rx, Qx_lx, Qx_rx, Qy_lx, Qy_rx, Qt_lx, Qt_rx, sigmatx_lx, sigmatx_rx, sigmaxx_lx, sigmaxx_rx, sigmaxy_lx, sigmaxy_rx, dens_lx, dens_rx, fxt_lxR, fxt_rxR, fxx_lxR, fxx_rxR, fxy_lxR, fxy_rxR, fdensx_lxR, fdensx_rxR)
    call flujos_yR(nx, ny, psi_ly, psi_ry, ux_ly, ux_ry, uy_ly, uy_ry, ut_ly, ut_ry, A_ly, A_ry, Qx_ly, Qx_ry, Qy_ly, Qy_ry, Qt_ly, Qt_ry, sigmaty_ly, sigmaty_ry, sigmaxy_ly, sigmaxy_ry, sigmayy_ly, sigmayy_ry, dens_ly, dens_ry, fyt_lyR, fyt_ryR, fyx_lyR, fyx_ryR, fyy_lyR, fyy_ryR, fdensy_lyR, fdensy_ryR)
...
!------------------------------------------------------------
    do j = 3, ny-3
      do i = 3, nx-3
            ...
            q11_rxR(i,j) = (exp(psi_rx(i,j))+A_rx(i,j))*(4.0d0/3.0d0*ut_rx(i,j)*ut_rx(i,j) - 1.0d0/3.0d0) + 2*ut_rx(i,j)*Qt_rx(i,j)
            ...
            q11_lxR(i,j) = (exp(psi_lx(i,j))+A_lx(i,j))*(4.0d0/3.0d0*ut_lx(i,j)*ut_lx(i,j) - 1.0d0/3.0d0) + 2*ut_lx(i,j)*Qt_lx(i,j)
            ...
            q11_ryR(i,j) = (exp(psi_ry(i,j))+A_ry(i,j))*(4.0d0/3.0d0*ut_ry(i,j)*ut_ry(i,j) - 1.0d0/3.0d0) + 2*ut_ry(i,j)*Qt_ry(i,j)
            ...
            q11_lyR(i,j) = (exp(psi_ly(i,j))+A_ly(i,j))*(4.0d0/3.0d0*ut_ly(i,j)*ut_ly(i,j) - 1.0d0/3.0d0) + 2*ut_ly(i,j)*Qt_ly(i,j)
...
!-------------------------------------------------------------
    ...
    call get_primitives(dt, eta, deltaeta, psi, dtpsi, dxpsi, dypsi, ux, dtux, dxux, dyux, uy, dtuy, dxuy, dyuy, ut, q11aux, q12aux, q13aux, dens, jtaux, nx, ny )
...
!------------------------------------------------------------
    ...
    do j = 4, ny-3
      ...
      ! q01(i,j) = q01(i,j) - dt/dx/2.0d0*(F0xtR(i,j)-F0xtR(i-1,j) + F0xtauxR(i,j)-F0xtauxR(i-1,j)) -dt/dy/2.0d0*(F0ytR(i,j)-F0ytR(i,j-1) + F0ytauxR(i,j)-F0ytauxR(i,j-1))
      ! q02(i,j) = q02(i,j) - dt/dx/2.0d0*(F0xxR(i,j)-F0xxR(i-1,j) + F0xxauxR(i,j)-F0xxauxR(i-1,j)) -dt/dy/2.0d0*(F0yxR(i,j)-F0yxR(i,j-1) + F0yxauxR(i,j)-F0yxauxR(i,j-1))
      ! q03(i,j) = q03(i,j) - dt/dx/2.0d0*(F0xyR(i,j)-F0xyR(i-1,j) + F0xyauxR(i,j)-F0xyauxR(i-1,j)) -dt/dy/2.0d0*(F0yyR(i,j)-F0yyR(i,j-1) + F0yyauxR(i,j)-F0yyauxR(i,j-1))
  
      jtaux2(i,j) = 3.0d0/4.0d0*jt(i,j) + 1.0d0/4.0d0*jtaux(i,j) - dt/dx/4.0d0*(FdensxR(i,j)-FdensxR(i-1,j)) -dt/dy/4.0d0*(FdensyR(i,j)-FdensyR(i,j-1))
  

      q11aux2(i,j) = 3.0d0/4.0d0*q11(i,j) + 1.0d0/4.0d0*q11aux(i,j) - dt/dx/4.0d0*(FxtR(i,j)-FxtR(i-1,j)) -dt/dy/4.0d0*(FytR(i,j)-FytR(i,j-1))
      q12aux2(i,j) = 3.0d0/4.0d0*q12(i,j) + 1.0d0/4.0d0*q12aux(i,j) - dt/dx/4.0d0*(FxxR(i,j)-FxxR(i-1,j)) -dt/dy/4.0d0*(FyxR(i,j)-FyxR(i,j-1))
      q13aux2(i,j) = 3.0d0/4.0d0*q13(i,j) + 1.0d0/4.0d0*q13aux(i,j) - dt/dx/4.0d0*(FxyR(i,j)-FxyR(i-1,j)) -dt/dy/4.0d0*(FyyR(i,j)-FyyR(i,j-1))
...
!-------------------------------------------------------------
      ...
      call get_primitives(dt, eta, deltaeta, psi, dtpsi, dxpsi, dypsi, ux, dtux, dxux, dyux, uy, dtuy, dxuy, dyuy, ut, q11aux2, q12aux2, q13aux2, dens, jtaux2, nx, ny )
  ...
  !------------------------------------------------------------
      ...
      do j = 4, ny-3
        ...
        jt(i,j) = 1.0d0/3.0d0*jt(i,j) + 2.0d0/3.0d0*jtaux2(i,j) - 2.0d0*dt/dx/3.0d0*(FdensxR(i,j)-FdensxR(i-1,j)) - 2.0d0*dt/dx/3.0d0*(FdensyR(i,j)-FdensyR(i,j-1))
    
  
        q11(i,j) = 1.0d0/3.0d0*q11(i,j) + 2.0d0/3.0d0*q11aux2(i,j) - 2.0d0*dt/dx/3.0d0*(FxtR(i,j)-FxtR(i-1,j)) - 2.0d0*dt/dx/3.0d0*(FytR(i,j)-FytR(i,j-1))
        q12(i,j) = 1.0d0/3.0d0*q12(i,j) + 2.0d0/3.0d0*q12aux2(i,j) - 2.0d0*dt/dx/3.0d0*(FxxR(i,j)-FxxR(i-1,j)) - 2.0d0*dt/dx/3.0d0*(FyxR(i,j)-FyxR(i,j-1))
        q13(i,j) = 1.0d0/3.0d0*q13(i,j) + 2.0d0/3.0d0*q13aux2(i,j) - 2.0d0*dt/dx/3.0d0*(FxyR(i,j)-FxyR(i-1,j)) - 2.0d0*dt/dx/3.0d0*(FyyR(i,j)-FyyR(i,j-1))
...
!-------------------------------------------------------------
    ...
    call get_primitives(dt, eta, deltaeta, psi, dtpsi, dxpsi, dypsi, ux, dtux, dxux, dyux, uy, dtuy, dxuy, dyuy, ut, q11, q12, q13, dens, jt, nx, ny )
