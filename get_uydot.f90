subroutine compute_dtuy(n, m, psi, ux, uy, dxpsi, dypsi, dxux, dyux, dxuy, dyuy, dtuy, eta, q11, q12, q13,deltaeta)
  implicit none
  ...
  do i = 4, n-3
    do j = 4, m-3
      ...
      ! PF correction for uy
      uyDpf = ( &
        -8.0d0*UXG**3*dxuy(i,j) &
        - UXG*(12.0d0*dxuy(i,j) + 4.0d0*UYG**2*(dyux(i,j) + 3.0d0*dxuy(i,j)) + UYG*dxpsi(i,j)) &
        - 2.0d0*UXG**2*(2.0d0*UYG*dyuy(i,j) + dypsi(i,j)) &
        - (1.0d0 + UYG**2)*(UYG*(-4.0d0*dxux(i,j) + 8.0d0*dyuy(i,j)) + 3.0d0*dypsi(i,j)) &
        ) / (4.0d0*sqrt(1.0d0+UXG**2+UYG**2)*(3.0d0+2.0d0*UXG**2+2.0d0*UYG**2))

      ! assemble final dtuy
      dtuy(i,j) = uyDpf &
        + (3.0d0*UYG*sqrt(1.0d0+UXG**2+UYG**2)*(4.0d0*ch + l + 2.0d0*(2.0d0*ch + l)*(UXG**2+UYG**2))*delta_tt) &
        ...
        - (UXG*UYG*(3.0d0*(ch + 6.0d0*ch*l + 2.0d0*l**2) + 6.0d0*(-1.0d0+l)*(2.0d0*ch + l)*UXG**4 + (2.0d0*l*(-1.0d0+6.0d0*l) + ch*(-9.0d0+29.0d0*l))*UYG**2 + 6.0d0*(-1.0d0+l)*(2.0d0*ch + l)*UYG**4 + UXG**2*(2.0d0*l*(-1.0d0+6.0d0*l) + ch*(-9.0d0+29.0d0*l) + 12.0d0*(-1.0d0+l)*(2.0d0*ch + l)*UYG**2)) * delta_tx) &
        ...
        + ((-3.0d0*(1.0d0 + UYG**2)*(l + (-1.0d0+l)*UYG**2)*(3.0d0*ch + 2.0d0*(2.0d0*ch + l)*UYG**2) - 2.0d0*UXG**4*(2.0d0*ch*(-3.0d0 + l) - 2.0d0*l + 3.0d0*(-1.0d0+l)*(2.0d0*ch + l)*UYG**2) + UXG**2*(-12.0d0*ch*(-1.0d0+l) + (ch*(33.0d0-37.0d0*l) + 2.0d0*(5.0d0-6.0d0*l)*l)*UYG**2 - 12.0d0*(-1.0d0+l)*(2.0d0*ch + l)*UYG**4)) * delta_ty) &
            / (exp(XIG)**0.75d0*(l + (-1.0d0+l)*(UXG**2+UYG**2))*(9.0d0*ch*l + 4.0d0*(ch*(-3.0d0+l)-l)*UXG**4 + 12.0d0*ch*(-1.0d0+l)*UYG**2 + 4.0d0*(ch*(-3.0d0+l)-l)*UYG**4 + 4.0d0*UXG**2*(3.0d0*ch*(-1.0d0+l)+2.0d0*(ch*(-3.0d0+l)-l)*UYG**2)))
    end do
  end do
  ...
  !write(*,*) "dtuy", maxval(dtuy)
end subroutine compute_dtuy
